define(function(require, exports, module){
	...
	var uploadImgFuc = function(uploadConfig){
		...
		var fileUploadItem = new fileUpload(uploadConfig);
			...
			fileUploadItem.on('swfLoaded', function(file, serverData, responseReceived){
				console.log("swfLoaded" +JSON.stringify(serverData))
		    });

		    fileUploadItem.on('dialogStart', function(file, serverData, responseReceived){
		    	console.log('dialogStart' + JSON.stringify(serverData))

		    });

		    fileUploadItem.on('fileQueued', function(file, serverData, responseReceived){
		    	console.log('fileQueued' + JSON.stringify(serverData))
		    });

		    fileUploadItem.on('queueError', function(file, serverData, responseReceived){
		    	console.log('queueError' + JSON.stringify(serverData))
		    });

		    fileUploadItem.on('dialogComplete', function(file, errorMessage){
		    	console.log('dialogComplete' + JSON.stringify(file))
		    });

		    fileUploadItem.on('uploadStart', function(file, serverData, responseReceived){
		    	console.log('uploadStart' + JSON.stringify(serverData))
		    });

		    fileUploadItem.on('uploadProgress', function(file, serverData, responseReceived){
		    	console.log('uploadProgress' + JSON.stringify(serverData))
		    });

		     //上传失败
		    fileUploadItem.on('uploadError', function(file, errorMessage){
		    	console.log('uploadError' + JSON.stringify(serverData))
		    });
		    
		    //上传成功
		    fileUploadItem.on('uploadSuccess', function(file, serverData, responseReceived){

		    	if(serverData.code && serverData.code == 200){
		    		pushImage(serverData.msg.imgurl);
		    	}
		    	console.log('uploadSuccess' + JSON.stringify(serverData))
		    });
		
		    fileUploadItem.on('uploadComplete', function(file, errorMessage){
		    	console.log('uploadComplete' + JSON.stringify(file))
		    });
	}

	var pushImage = function(url){
		var htmls = document.getElementById("edit-img-box").innerHTML;
		var str = htmls + "<img src=" + "'" + url + "'/>";
		document.getElementById("edit-img-box").innerHTML = str;
	}
	...
	TextEditor.prototype = {
		...
		imgagePickerHtml: function(){
			...
			builder.push('<div class="img-box" id="edit-img-box"></div><div class="uploadbtn" id="upload-box" stlye="display:block"><button id="uploadbtn">选择图片</button></div>');
			builder.push('<div class="btn-box"><button class="" id="edit_btn">确定</button></div>');
		...
		drawTextEditor: function(id){
			...
			var buttonClone = $.create("a"),
				...
				$.addEvent(toolBar['insertimage'],'click',function(){
					...
					$.addEvent(document.getElementById('edit_btn'),'click',function(){
						var imgList = $.ClassName("img-box")[0].innerHTML;
						var htmls = iframeDoment.body.innerHTML;
						iframeDoment.body.innerHTML = htmls + imgList;
						$.ClassName('insertImage')[0].style.display = "none";
						$.ClassName("img-box")[0].innerHTML = "";
					})
				})
